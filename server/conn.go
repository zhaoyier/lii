...
func asyncWrite(c *ServerConn, m Message) (err error) {
	defer func() {
		if p := recover(); p != nil {
			err = ErrServerClosed
		}
...
func readLoop(c *ServerConn, wg *sync.WaitGroup) {
	...
	//TODO onMessage = c.belong.opts.onMessage
	...
	for {
		...
		default:
			...
			//	continue
			//}
			handlerCh <- MessageHandler{msg, ProcessMessage}
		}
	}
}
